...
import { createProtomapsStyle, PROTOMAPS_ATTRIBUTION } from "./protomaps";
...
describe("createProtomapsStyle", () => {
  ...
  it("defaults the attribution to PROTOMAPS_ATTRIBUTION", () => {
    const style = createProtomapsStyle({ url: "pmtiles:///x.pmtiles" });
    const source = style.sources.protomaps!;
    if (source.type === "vector") {
      expect(source.attribution).toBe(PROTOMAPS_ATTRIBUTION);
    }
  ...
  it("lets callers override the attribution at init time", () => {
    const style = createProtomapsStyle({
      url: "pmtiles:///x.pmtiles",
    ...
    const source = style.sources.protomaps!;
    if (source.type === "vector") {
      expect(source.attribution).toBe("© My Company");
    }
  });
});
