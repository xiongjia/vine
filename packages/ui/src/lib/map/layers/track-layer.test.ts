...
import { syncTrackLayer } from "./track-layer";
import type { TrackFeature } from "./track-layer";
...
describe("syncTrackLayer", () => {
  it("adds one source + line layer per track with index-based ids", () => {
    ...
    const cleanup = syncTrackLayer(map as never, [
      track("A", "#111111"),
      track("B", "#222222"),
    ]);
    ...
    expect(
      (map.addSource.mock.calls[0]![1] as { data: TrackFeature }).data.geometry
        .type,
    ).toBe("LineString");
  ...
  it("keeps ids distinct even when track names collide", () => {
    const map = createMockMap();
    syncTrackLayer(map as never, [
      track("same", "#111111"),
      track("same", "#222222"),
    ]);
    expect(
      map.addLayer.mock.calls.map((c) => (c[0] as { id: string }).id),
    ).toEqual(["track-0", "track-1"]);
  });

  it("waits for the style to load and re-applies on style.load", () => {
  ...
  it("re-applies after a style reload (setStyle) — idempotent", () => {
    const map = createMockMap();
    syncTrackLayer(map as never, [track("A", "#111111")]);
    expect(map.addSource).toHaveBeenCalledTimes(1);

    // a later Map.setStyle() reloads the style and fires style.load again
    map.fire("style.load");
    expect(map.addSource).toHaveBeenCalledTimes(2);
  ...
  it("re-apply removes the previous sources/layers (idempotent)", () => {
    ...
    expect(
      (map.addLayer.mock.calls[1]![0] as { paint: { "line-color": string } })
        .paint["line-color"],
    ).toBe("#222222");
  });

  it("cleanup stops pending style.load work", () => {
    const map = createMockMap(false);
